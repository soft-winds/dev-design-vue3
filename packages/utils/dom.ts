...
import raf from "raf";
import { isString, isFunction, isArray } from "lodash-es";
import { easeInOutCubic, type EasingFunction } from "./easing";
type ScrollContainer = any;
type ScrollContainerElement = Window | HTMLElement | null;
...
export const getScrollContainer = (
  container: ScrollContainer = "body"
...
function containerDom(
  parent: VNode | Element | Iterable<any> | ArrayLike<any>,
  child: any
): boolean {
  if (parent && child) {
    let pNode = child;
...
export const clickOut = (
  els: VNode | Element | Iterable<any> | ArrayLike<any>,
  cb: () => void
): void => {
  on(document, "click", (event: { target: Element }) => {
    if (isArray(els)) {
      const isFlag = Array.from(els).every(
        (item) => containerDom(item, event.target) === false
      );
      return isFlag && cb && cb();
    }
...
export const scrollSelectedIntoView = (
 ...
 * @returns string
 */
export function getElmCssPropValue(
  element: HTMLElement,
  propName: string
): string {
  let propValue = "";
...
export function getOffsetTop(target: HTMLElement, container: any): number {
